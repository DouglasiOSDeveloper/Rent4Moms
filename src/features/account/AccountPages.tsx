...
export function AccountOrderDetail() {
  const { quoteId = "" } = useParams(); const routerNavigate = useNavigate(); const { products, assemblyVariants, refreshCatalog } = useCatalog();
  ...
  return <div className="space-y-5"><button type="button" onClick={() => routerNavigate(-1)} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft size={14} />Voltar</button>
    ...
    <Panel title="Precisa de ajuda?"><div className="grid sm:grid-cols-2 gap-3"><label className="text-sm font-medium">Assunto<input value={supportSubject} onChange={(event) => setSupportSubject(event.target.value)} aria-describedby="support-subject-help" className="mt-1.5 w-full rounded-xl border border-border bg-input-background px-3 py-2.5" /><span id="support-subject-help" className="mt-1 block text-xs text-muted-foreground">Mínimo de 3 caracteres · {supportSubject.trim().length}/3</span></label><label className="text-sm font-medium">Mensagem<textarea value={supportMessage} onChange={(event) => setSupportMessage(event.target.value)} aria-describedby="support-message-help" className="mt-1.5 w-full rounded-xl border border-border bg-input-background px-3 py-2.5 min-h-24" /><span id="support-message-help" className="mt-1 block text-xs text-muted-foreground">Mínimo de 5 caracteres · {supportMessage.trim().length}/5</span></label></div><div className="mt-3 flex justify-end"><Btn variant="outline" size="sm" disabled={supportSaving || supportSubject.trim().length < 3 || supportMessage.trim().length < 5} onClick={() => void submitSupport()}>{supportSaving ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />}Enviar para o atendimento</Btn></div>{detail.supportRequests.length > 0 && <div className="mt-4 space-y-2">{detail.supportRequests.map((item) => <div key={item.id} className="rounded-xl bg-secondary p-3"><div className="flex justify-between gap-2"><p className="text-sm font-medium">{item.subject}</p><StatusBadge status={SUPPORT_LABELS[item.status]} /></div><p className="text-sm text-muted-foreground mt-1">{item.message}</p>{item.adminNote && <p className="text-sm mt-2"><strong>Resposta da equipe:</strong> {item.adminNote}</p>}</div>)}</div>}</Panel>
  </div>;
}
