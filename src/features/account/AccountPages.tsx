...
export function AccountOrderDetail() {
  ...
  return <div className="space-y-5"><button type="button" onClick={() => routerNavigate(-1)} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft size={14} />Voltar</button>
    ...
    <Panel title="Fotos da operação">{detail.attachments.length ? <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{detail.attachments.map((attachment) => <figure key={attachment.id} className="rounded-xl overflow-hidden border border-border"><img src={attachment.contentUrl} alt={attachment.note || attachment.originalName} className="w-full h-40 object-cover bg-secondary" /><figcaption className="p-3 text-xs text-muted-foreground">{attachment.kind === "delivery" ? "Entrega" : attachment.kind === "return" ? "Devolução" : "Documento"} · {formatDateTime(attachment.createdAt)}</figcaption></figure>)}</div> : <p className="text-sm text-muted-foreground">A equipe ainda não publicou fotos para este pedido.</p>}</Panel>
