...
export function AdminOrderDetail() {
  ...
  return <div className="space-y-5">
    ...
    <Panel title="Itens contratados" icon={<Package size={17} className="text-primary"/>}><div className="space-y-3">{quote.payload.items.map((item)=><article key={item.id} className="flex flex-col sm:flex-row gap-4 rounded-xl border border-border p-4"><img src={item.productSnapshot.photo} alt={item.productSnapshot.name} className="h-24 w-24 rounded-lg object-cover bg-secondary"/><div className="flex-1"><p className="font-medium">{item.productSnapshot.name}</p><p className="text-sm text-muted-foreground">{item.productSnapshot.assembly?`${item.productSnapshot.assembly.cover.name}${item.productSnapshot.assembly.reducer?` + ${item.productSnapshot.assembly.reducer.name}`:" + sem redutor"}`:item.productSnapshot.description}</p><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted-foreground"><span>{item.quantity} unidade(s)</span><span>{item.periodDays} dias</span><span>{item.startDate} até {item.endDate}</span></div></div><p className="font-semibold">{formatMoneyFromCents(item.priceSnapshot.totalCents)}</p></article>)}</div></Panel>
