...
export function AdminInventory() {
  ...
  const itemOptions = useMemo(() => ({
    ...
    cover: covers.filter((item) => item.isActive).map((item) => ({ id: item.id, label: item.name })),
    reducer: reducers.filter((item) => item.isActive).map((item) => ({ id: item.id, label: item.name })),
    ball_set: ballSets.filter((item) => item.isActive).map((item) => ({ id: item.id, label: item.name })),
  }), [ballSets, chairModels, covers, reducers]);
  const hasCatalogItems = Object.values(itemOptions).some((items) => items.length > 0);
  ...
  return <div>
    <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
      ...
      <div className="flex gap-2"><Btn variant="outline" size="sm" onClick={() => void expire()} disabled={busy}><RefreshCw size={14} />Liberar vencidos</Btn><Btn variant="primary" size="sm" onClick={() => setEditing("new")} disabled={!hasCatalogItems}><Plus size={14} />Nova unidade</Btn></div>
    </div>
    ...
    {!hasCatalogItems && <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Cadastre e ative um modelo, pano, redutor ou conjunto de bolinhas antes de criar unidades físicas.</div>}
    ...
    {tab === "units" && <div className="bg-white rounded-xl border border-border">
      ...
      {loading ? <div className="p-10 text-center text-muted-foreground">Carregando estoque...</div> : <div className="overflow-x-auto"><table className="w-full"><thead className="bg-secondary border-b border-border"><tr>{["Código","Tipo","Item","Status","Condição","Localização","Atualização",""].map((h)=><th key={h} className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap">{h}</th>)}</tr></thead><tbody className="divide-y divide-border">{filtered.map((unit)=><tr key={unit.id} className="hover:bg-secondary/50"><td className="px-4 py-3 text-xs font-mono font-medium">{unit.code}</td><td className="px-4 py-3 text-sm text-muted-foreground">{TYPE_LABELS[unit.itemType]}</td><td className="px-4 py-3"><p className="text-sm">{unit.catalogItemLabel ?? unit.label}</p><p className="text-xs text-muted-foreground font-mono">{unit.itemId}</p>{unit.catalogReferenceStatus !== "active" && <p className={cn("mt-1 text-xs", unit.catalogReferenceStatus === "missing" ? "text-destructive" : "text-amber-700")}>{unit.catalogReferenceStatus === "missing" ? "Referência de catálogo ausente" : "Item de catálogo inativo"}</p>}</td><td className="px-4 py-3"><StatusBadge status={STATUS_LABELS[unit.status]}/></td><td className="px-4 py-3 text-sm text-muted-foreground">{unit.condition}</td><td className="px-4 py-3 text-sm text-muted-foreground">{unit.location}</td><td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">{dateTime(unit.updatedAt)}</td><td className="px-4 py-3"><div className="flex gap-1"><button onClick={()=>setEditing(unit)} title="Editar" className="p-1.5 rounded-lg text-muted-foreground hover:bg-secondary"><Edit size={14}/></button><button onClick={()=>void retire(unit)} title="Baixar" disabled={busy||unit.status==='retired'} className="p-1.5 rounded-lg text-muted-foreground hover:bg-red-50 hover:text-destructive disabled:opacity-40"><Trash2 size={14}/></button></div></td></tr>)}{filtered.length===0&&<tr><td colSpan={8} className="p-10 text-center text-muted-foreground">Nenhuma unidade encontrada.</td></tr>}</tbody></table></div>}
